...
def get_investigation_data():
    ...
    topology_nodes = [TopologyNodeResponse(**node) for node in SAMPLE_TOPOLOGY["nodes"]]
    topology_edges = [TopologyEdgeResponse(**edge) for edge in SAMPLE_TOPOLOGY["edges"]]
    hypothesis_tree = HypothesisTreeResponse(root=SAMPLE_HYPOTHESIS["root"])
