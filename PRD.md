...
| 能力类别 | 必须上线能力 | 验收标准（摘要） |
|---------|-------------|----------------|
| Agent 主链路 | Agent 引擎、执行模式管理（Direct/Plan-Execute/Hierarchical 至少两种可用） | 可完成端到端诊断任务并输出根因结论 |
| 工具与上下文 | 内置日志分析 + 代码搜索 + 文件读写 + 上下文管理 | 诊断过程可回放，关键证据可追溯 |
| 可靠性 | 故障处理（重试、超时、失败恢复）+ 断点恢复（最小粒度到 Agent 节点） | 注入故障后任务可重试或从断点恢复 |
| 安全与治理 | Skill 管理（上传/解析）+ Skill 沙盒隔离 + 基础审计日志 | 非授权 Skill 无法越权访问宿主文件系统/网络 |
...
| 能力类别 | 架构预留能力 | 当前要求 |
|---------|-------------|---------|
| 生态扩展 | MCP 管理、第三方工具市场化接入 | 保留接口与配置模型，不要求在 MVP 完成全量对接 |
| 诊断增强 | 调用链追踪、修复建议自动化、业务规范深度匹配 | 保留 Agent/Tool 扩展点与数据结构 |
| 知识沉淀 | 知识库自动学习、问题模式自动聚类 | MVP 可先手工维护知识条目 |
| 平台能力 | 多租户、复杂审批流、全量 RBAC 细粒度策略 | 单租户 + 基础角色满足首发 |
