...
export default function HistoricalCases({ cases, onEdit, onDelete }: HistoricalCasesProps) {
  return (
    <Card className="p-6">
      ...
      {cases.length === 0 && (
        <div className="p-8 rounded-lg border border-dashed border-border-subtle text-center">
          <p className="text-text-main text-sm">暂无历史案例</p>
          <p className="text-text-muted text-xs mt-1">请先新增案例，或稍后刷新数据。</p>
        </div>
      )}

      <div className="space-y-3">
        {cases.map((caseItem) => {
          const detailPath = `/knowledge/cases/${caseItem.id}`;
            ...
            <div key={caseItem.id} className="p-4 bg-bg-elevated/30 rounded-lg">
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1">
                  {caseItem.id ? (
                    <Link to={detailPath} className="text-sm font-medium text-text-main mb-2 inline-block hover:text-primary hover:underline">
                      {caseItem.title}
                    </Link>
                  ) : (
                  ...
                  <div className="flex items-center gap-4 text-xs text-text-muted mt-2">
                    <span>ID: {caseItem.id || '未知（无法跳转详情）'}</span>
